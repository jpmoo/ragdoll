...
echo ""
echo "Optional: install MCP server (SSE mode). Requires: pip install -e '.[mcp]' and RAGDOLL_MCP_TRANSPORT=sse in env.ragdoll"
echo "  sudo cp $BIN/ragdoll-mcp.service /etc/systemd/system/"
echo "  sudo systemctl daemon-reload && sudo systemctl enable --now ragdoll-mcp"
echo ""
